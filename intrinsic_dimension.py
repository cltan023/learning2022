...
def id_estimate(feature_maps):
    """ estimate intrinsic dimension of features

    Args:
        feature_maps (Dict): each key implies an intermediate layer
    """   
    dist = torch.pdist(feature_maps)
    try:
        reg = estimate(squareform(dist.numpy()))
    except Exception:
        reg = -1.0
    del dist
