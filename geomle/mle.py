...
def intrinsic_dim_sample_wise(X, k=5, dist = None):
    ...
    """
    if dist is None:
        neighb = NearestNeighbors(n_neighbors=k+1, n_jobs=CPU_COUNT, algorithm='ball_tree').fit(X)
...
def intrinsic_dim_scale_interval(X, k1=10, k2=20, dist = None):
    ...
    intdim_k = []
    if dist is None:
        neighb = NearestNeighbors(n_neighbors=k2+1, n_jobs=CPU_COUNT, algorithm='ball_tree').fit(X)
        dist, ind = neighb.kneighbors(X)
